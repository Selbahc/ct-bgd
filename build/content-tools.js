...
(function() {
  ...
  ContentTools.LinkDialog = (function(_super) {
    ...
    LinkDialog.prototype.save = function() {
      ...
      detail = {
        href: this._domInput.value.length > 0 && this._domInput.value.indexOf('http') === -1 ? 'http://' + this._domInput.value.trim() : this._domInput.value.trim()
      };
      if (this._target) {
  ...
  ContentTools.VideoAudioDialog = (function(_super) {
    __extends(VideoAudioDialog, _super);

    function VideoAudioDialog() {
      VideoAudioDialog.__super__.constructor.call(this, 'Insert audio or video file');
      this._fileURL = null;
      this._fileType = null;
      this._progress = 0;
      this._state = 'empty';
    ...
    VideoAudioDialog.prototype.clear = function() {
      if (this._domFile) {
        this._domFile.parentNode.removeChild(this._domFile);
        this._domFile = null;
      }
      this._fileURL = null;
      this._fileType = null;
    ...
    VideoAudioDialog.prototype.mount = function() {
      var domActions, domProgressBar, domTools;
      VideoAudioDialog.__super__.mount.call(this);
      ContentEdit.addCSSClass(this._domElement, 'ct-media-dialog');
      ContentEdit.addCSSClass(this._domElement, 'ct-media-dialog--empty');
      ContentEdit.addCSSClass(this._domView, 'ct-media-dialog__view');
      domTools = this.constructor.createDiv(['ct-control-group', 'ct-control-group--left']);
      this._domControls.appendChild(domTools);
      domProgressBar = this.constructor.createDiv(['ct-progress-bar']);
      domTools.appendChild(domProgressBar);
      ...
      domActions.appendChild(this._domUpload);
      this._domInput = document.createElement('input');
      this._domInput.setAttribute('class', 'ct-media-dialog__file-upload');
      this._domInput.setAttribute('name', 'file');
      this._domInput.setAttribute('type', 'file');
      this._domInput.setAttribute('accept', 'audio/mpeg,video/mp4');
      this._domUpload.appendChild(this._domInput);
      this._domInsert = this.constructor.createDiv(['ct-control', 'ct-control--text', 'ct-control--insert']);
      this._domInsert.textContent = ContentEdit._('Insert');
      domActions.appendChild(this._domInsert);
      ...
      domActions.appendChild(this._domClear);
      this._addDOMEventListeners();
      return this.dispatchEvent(this.createEvent('fileuploader.mount'));
    };

    VideoAudioDialog.prototype.progress = function(progress) {
      if (progress === void 0) {
        return this._progress;
      }
      this._progress = progress;
    ...
    VideoAudioDialog.prototype.save = function(fileURL, fileType) {
      return this.dispatchEvent(this.createEvent('save', {
        'fileURL': fileURL,
        'fileType': fileType
      }));
    ...
    VideoAudioDialog.prototype.state = function(state) {
      var prevState;
      if (state === void 0) {
        return this._state;
      }
      ...
      ContentEdit.addCSSClass(this._domElement, "ct-media-dialog--" + this._state);
      return ContentEdit.removeCSSClass(this._domElement, "ct-media-dialog--" + prevState);
    };

    VideoAudioDialog.prototype.unmount = function() {
      VideoAudioDialog.__super__.unmount.call(this);
      this._domCancelUpload = null;
      this._domClear = null;
      this._domInput = null;
      this._domInsert = null;
      this._domProgress = null;
      this._domUpload = null;
      return this.dispatchEvent(this.createEvent('fileuploader.unmount'));
    };

    VideoAudioDialog.prototype.populate = function(fileURL, fileType) {
      this._fileURL = fileURL;
      this._fileType = fileType;
      if (this._fileType === 'video/mp4') {
        this._domFile = document.createElement('video');
        this._domFile.setAttribute('height', '100%');
        this._domFile.setAttribute('src', this._fileURL);
        this._domFile.setAttribute('controls', '');
        this._domFile.setAttribute('width', '100%');
        this._domView.appendChild(this._domFile);
      }
      if (this._fileType === 'audio/mpeg') {
        this._domFile = document.createElement('audio');
        this._domFile.setAttribute('src', this._fileURL);
        this._domFile.setAttribute('controls', '');
        this._domFile.setAttribute('style', 'width: 100%; height: 100%;');
        this._domView.appendChild(this._domFile);
      }
      return this.state('populated');
    };

    VideoAudioDialog.prototype._addDOMEventListeners = function() {
      VideoAudioDialog.__super__._addDOMEventListeners.call(this);
      this._domInput.addEventListener('change', (function(_this) {
        return function(ev) {
          var file;
          file = ev.target.files[0];
          ...
          return _this.dispatchEvent(_this.createEvent('fileuploader.fileready', {
            file: file
          }));
        };
      })(this));
      this._domCancelUpload.addEventListener('click', (function(_this) {
        return function(ev) {
          return _this.dispatchEvent(_this.createEvent('fileuploader.cancelupload'));
        };
      })(this));
      this._domClear.addEventListener('click', (function(_this) {
        return function(ev) {
          return _this.dispatchEvent(_this.createEvent('fileuploader.clear'));
        };
      })(this));
      return this._domInsert.addEventListener('click', (function(_this) {
        return function(ev) {
          return _this.dispatchEvent(_this.createEvent('fileuploader.save'));
        };
      })(this));
    };

    return VideoAudioDialog;
  ...
  ContentTools.Tools.Video = (function(_super) {
    ...
    Video.label = 'Lien Viméo ou Youtube';
  ...
  ContentTools.Tools.VideoAudio = (function(_super) {
    __extends(VideoAudio, _super);

    function VideoAudio() {
      return VideoAudio.__super__.constructor.apply(this, arguments);
    }

    ContentTools.ToolShelf.stow(VideoAudio, 'video-audio');

    VideoAudio.label = 'Fichier Vidéo ou Audio';

    VideoAudio.icon = 'file-play';

    VideoAudio.canApply = function(element, selection) {
      if (element.isFixed()) {
        if (element.type() !== 'ImageFixture') {
          return false;
        }
    ...
    VideoAudio.apply = function(element, selection, callback) {
      var app, dialog, modal, toolDetail;
      toolDetail = {
        'tool': this,
        'element': element,
      ...
      app = ContentTools.EditorApp.get();
      modal = new ContentTools.ModalUI();
      dialog = new ContentTools.VideoAudioDialog();
      dialog.addEventListener('cancel', (function(_this) {
        return function() {
          modal.hide();
          dialog.hide();
      ...
      dialog.addEventListener('save', (function(_this) {
        return function(ev) {
          var detail, fileElt, fileType, fileURL, index, node, _ref;
          detail = ev.detail();
          ...
          if (element.type() === 'ImageFixture') {
            ...
            if (fileType === 'video/mp4') {
              fileElt = new ContentEdit.Video('video', {
                'controls': '',
                'height': ContentTools.DEFAULT_VIDEO_HEIGHT,
                'src': fileURL,
                'width': ContentTools.DEFAULT_VIDEO_WIDTH
            ...
            if (fileType === 'audio/mpeg') {
              fileElt = new ContentEdit.Video('audio', {
                'controls': '',
                'data-ce-moveable': '',
                'style': "width:" + ContentTools.DEFAULT_VIDEO_WIDTH + "px; min-height: 40px;",
                'src': fileURL
              });
            }
            _ref = _this._insertAt(element), node = _ref[0], index = _ref[1];
            node.parent().attach(fileElt, index);
            fileElt.focus();
          }
          modal.hide();
          dialog.hide();

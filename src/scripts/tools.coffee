...
class ContentTools.Tools.Underline extends ContentTools.Tools.Bold
    ...
    # Make the current selection of text (non)underline (e.g <u>foo</u>).
...
class ContentTools.Tools.VideoAudio extends ContentTools.Tool

    # Insert an audio or video file.

    ContentTools.ToolShelf.stow(@, 'video-audio')
        ...
        dialog.addEventListener 'save', (ev) =>
            ...
            if element.type() is 'ImageFixture'
                ...
                element.src(fileURL)
                 ...
                 # Insert the video
                if fileType is 'video/mp4'
                    fileElt = new ContentEdit.Video(
                        'video', {
                            'controls': '',
                            'height': ContentTools.DEFAULT_VIDEO_HEIGHT,
                            'src': fileURL,
